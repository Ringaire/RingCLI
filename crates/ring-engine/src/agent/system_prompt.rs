// 系统提示词构建：注入环境、git 状态、工具与技能清单、Memory、AGENTS.md、SYSTEM.md

use std::path::Path;

use ring_core::{build_memory_prompt, list_memory};
...
/// 发现 SYSTEM.md（自定义系统提示词）。
/// 优先级：项目 .ring/SYSTEM.md > 项目 .ring/SYSTEM.md > 全局 ~/.ring/config/SYSTEM.md
pub fn discover_system_prompt_file(cwd: &Path) -> Option<std::path::PathBuf> {
    // 项目级 .ring/SYSTEM.md
    let p = cwd.join(".ring").join("SYSTEM.md");
    if p.is_file() { return Some(p); }
    // 项目级 .ring/SYSTEM.md（兼容）
    let p = cwd.join(".ring").join("SYSTEM.md");
    if p.is_file() { return Some(p); }
    // 全局
    let p = paths::config_dir().join("SYSTEM.md");
    if p.is_file() { return Some(p); }
    None
}

/// 发现 APPEND_SYSTEM.md（追加到系统提示词末尾）。
/// 优先级：项目 .ring/APPEND_SYSTEM.md > 项目 .ring/APPEND_SYSTEM.md > 全局 ~/.ring/config/APPEND_SYSTEM.md
pub fn discover_append_system_prompt_file(cwd: &Path) -> Option<std::path::PathBuf> {
    let p = cwd.join(".ring").join("APPEND_SYSTEM.md");
    if p.is_file() { return Some(p); }
    let p = cwd.join(".ring").join("APPEND_SYSTEM.md");
    if p.is_file() { return Some(p); }
    let p = paths::config_dir().join("APPEND_SYSTEM.md");
    if p.is_file() { return Some(p); }
    None
}

/// 发现 CLAUDE.md（兼容 Pi / Claude Code 项目）。
/// 从 cwd 向上查找，最多 10 层，遇到 git 根即停。
...
pub async fn build_system_prompt(
    ...
    // ── 身份 ──
    s.push_str("You are ring, a terminal-based AI coding assistant. ");
    ...
    s.push_str(&format!("- ring version: {}\n", env!("CARGO_PKG_VERSION")));
    ...
    // ── CLAUDE.md（兼容 Pi / Claude Code 项目）──
    if let Some(claude_md) = read_claude_md(cwd) {
        let trimmed = claude_md.trim();
        if !trimmed.is_empty() {
            s.push_str("# Project Instructions (CLAUDE.md)\n");
            s.push_str(trimmed);
            s.push_str("\n\n");
        }
    }

    // ── SYSTEM.md（自定义系统提示词，替换默认行为准则）──
    if let Some(path) = discover_system_prompt_file(cwd) {
        if let Ok(content) = std::fs::read_to_string(&path) {
            let trimmed = content.trim();
            if !trimmed.is_empty() {
                s.push_str(&format!("# Custom System Prompt ({})\n", path.display()));
                s.push_str(trimmed);
                s.push_str("\n\n");
            }
        }
    ...
    // ── APPEND_SYSTEM.md（追加到系统提示词末尾，最高优先级）──
    if let Some(path) = discover_append_system_prompt_file(cwd) {
        if let Ok(content) = std::fs::read_to_string(&path) {
            let trimmed = content.trim();
            if !trimmed.is_empty() {
                s.push_str(&format!("# Append System ({})\n", path.display()));
                s.push_str(trimmed);
                s.push_str("\n\n");
            }
        }
