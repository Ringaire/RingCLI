...
impl AgentExecutor {
    ...
    async fn do_turn(&self, ctx: &mut AgentContext, signal: CancellationToken) -> TurnResult {
        ...
        let req = ChatRequest {
            ...
            tools:         self.tools.list().iter().map(|t| ring_providers::provider::ToolDef {
        ...
        for (idx, (call_id, tool_name, input_json)) in tool_calls.iter().enumerate() {
            ...
            let action = {
                ...
                let restricted = matches!(
                    perms.mode(),
                    ring_core::permissions::ModeName::Ask
                        | ring_core::permissions::ModeName::Edit
                        | ring_core::permissions::ModeName::Plan
                );
        ...
        for block in msg.content.iter_mut() {
            if let ContentBlock::ToolResult { tool_result, .. } = block {
                ...
                if s.len() > TRUNCATE_TO * 2 {
                    // Find a valid UTF-8 char boundary at or before TRUNCATE_TO
                    let mut end_idx = TRUNCATE_TO.min(s.len());
