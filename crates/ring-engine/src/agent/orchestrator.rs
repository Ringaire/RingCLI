...
use ring_core::agent::{ModelCatalogEntry, ModelRole};
use ring_core::events::EventBus;
use ring_core::permissions::DefaultPermissionEngine;
use ring_core::tools::{AugmentedToolRegistry, ToolRegistry};
...
pub struct ExecutorParams {
    ...
    pub perm_tx:          Option<PermissionSender>,
}

/// 构建主 agent 的 orchestrator executor：在基础工具集之上叠加 spawn_agent。
pub fn build_executor(params: ExecutorParams) -> AgentExecutor {
    let ExecutorParams {
        tools,
        permissions,
        bus,
