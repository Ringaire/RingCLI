...
//! `.agent/Neko-all/nekocli/linkkit-implementation-design.md`。
//!
//! 依赖方向：`neko-linkkit` → `linkkit-parser`（仅解析层，零 IO）。
//! 上层 `neko-engine` 负责把解析结果接入 agent executor。
