...
//! 7. 持久化到 ~/.config/ring/auth.json（权限 0600）
//!
...
/// 持久化的认证数据（~/.config/ring/auth.json）。
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct AuthData {
...
pub async fn login_browser(client: &Client) -> Result<AuthData, OAuthError> {
    ...
    // 打开浏览器
    let _ = open_browser(&url).map_err(|e| {
...
async fn wait_for_callback(
    ...
    let _deadline = tokio::time::Instant::now() + timeout;
    ...
    let (status_html, status_msg) = if params.contains_key("error") {
...
/// auth.json 的存储路径：~/.config/ring/auth.json
pub fn auth_file_path() -> PathBuf {
    ring_core::session::paths::config_dir().join("auth.json")
}
...
<html><head><title>ring</title><meta charset="utf-8"></head>
