...
//! 优先级（高 → 低）：项目级 `.ring/providers.json` > 全局 `~/.config/ring/providers.json` > 内置默认
...
pub fn load(
    ...
    if let Some(dir) = project_dir {
        merge_file(&mut catalog, &dir.join(".ring").join("providers.json"));
    }
...
pub fn defaults() -> HashMap<String, CatalogEntry> {
    ...
    m.insert("openai-responses".into(), providers::openai::responses::catalog_entry());
    ...
    // T0 新增：模型平台 & 中国区
    m.insert("huggingface".into(),          added::huggingface::catalog_entry());
    m.insert("minimax".into(),              added::minimax::catalog_entry());
    m.insert("minimax-cn".into(),           added::minimax_cn::catalog_entry());
    m.insert("qwen".into(),                 added::qwen::catalog_entry());
    m.insert("qwen-cn".into(),              added::qwen_cn::catalog_entry());
    m.insert("xiaomi".into(),               added::xiaomi::catalog_entry());
    m.insert("xiaomi-ams".into(),           added::xiaomi_ams::catalog_entry());
    m.insert("xiaomi-sgp".into(),           added::xiaomi_sgp::catalog_entry());
    m.insert("zai".into(),                  added::zai::catalog_entry());
    m.insert("zai-cn".into(),               added::zai_cn::catalog_entry());
    m.insert("ant-ling".into(),             added::ant_ling::catalog_entry());
    m.insert("cloudflare-workers".into(),   added::cloudflare_workers::catalog_entry());
    m.insert("cloudflare-gateway".into(),  added::cloudflare_gateway::catalog_entry());
...
mod tests {
    ...
    fn all_well_known_providers_present() {
        ...
        for id in &[
            ...
            // T0 新增
            "huggingface", "minimax", "minimax-cn",
            "qwen", "qwen-cn",
            "xiaomi", "xiaomi-ams", "xiaomi-sgp",
            "zai", "zai-cn", "ant-ling",
            "cloudflare-workers", "cloudflare-gateway",
            "vercel-gateway", "opencode",
        ] {
