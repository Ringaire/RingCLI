...
//! 与全局 `~/.ring/` 配置/session 存储。tokio 测试属性默认并行执行，
...
fn create_temp_dir_with_config(config_content: &str) -> std::io::Result<TempDir> {
    ...
    let config_path = ring_dir.join("settings.jsonc");
...
#[tokio::test]
#[ignore = "bootstrap reads global std::env + ~/.ring; refactor to DI in stage 2"]
async fn test_bootstrap_cold_start_no_provider() {
    ...
    let result = ring_cli::bootstrap::bootstrap(&args, None).await;
...
async fn test_bootstrap_tools_initialization() {
    ...
    // 验证一些预期的内置工具存在（根据 ring-tools 的实现）
...
async fn test_bootstrap_skills_loading() {
    ...
    // 创建 .ring/skills 目录
    let skills_dir = temp.path().join(".ring").join("skills");
...
"#).expect("failed to write skill file");
    ...
    assert!(skills.iter().any(|s| s.name == "test-skill"),
            "test skill should be loaded from .ring/skills");
}
...
async fn test_bootstrap_config_merge_order() {
    ...
    let ring_dir = temp.path().join(".ring");
    std::fs::create_dir_all(&ring_dir).expect("failed to create .ring dir");
    std::fs::write(ring_dir.join("settings.jsonc"), config).expect("failed to write config");
