...
use ring_engine::agent::tool_preview::extract_tool_preview;
use crate::args::Args;
use crate::bootstrap::BootstrappedRuntime;
use crate::repl::history::History;

/// Provider setup channel type alias.
type SetupTx = mpsc::Sender<Result<(Vec<String>, Option<String>), String>>;
...
struct AppState {
    ...
    /// thinking token 预算
    think_budget:     u32,
    /// reasoning effort 级别（off/minimal/low/medium/high/xhigh/max），None = 不发送
    effort:           Option<String>,
    /// 自主循环状态（/loop 命令）
    loop_state:       Option<ring_core::session::loop_state::LoopState>,
...
impl AppState {
    ...
    fn load_history_messages(&mut self, messages: &[Message]) {
        use ring_core::tools::{ContentBlock, MessageRole};
        for m in messages {
            match m.role {
                MessageRole::User => {
...
fn collect_text(content: &[ring_core::tools::ContentBlock]) -> String {
...
async fn run_loop<B: ratatui::backend::Backend>(
    ...
    if runtime.provider.is_none() {
        state.chat.add_system("Setup Required — ring needs a model provider before it can start.");
            ...
            Some(result) = done_rx.recv() => {
                ...
                if !state.queued_messages.is_empty() {
                    ...
                    let done = state.chat.bubbles.last()
                        ...
                        .map(|b| b.content.contains(ring_core::LOOP_DONE_MARKER))
                        .unwrap_or(false);
                    if done {
    ...
    match ev {
        ...
        Event::Key(ke) if ke.kind == KeyEventKind::Press => {
            ...
            if state.effort_picker.is_some() {
                return handle_effort_picker_key(ke, state).await;
            }
            // 登出选择器优先处理按键
...
async fn handle_effort_picker_key(
    ...
    match ke.code {
        KeyCode::Up => {
            if let Some(picker) = &mut state.effort_picker {
                picker.move_up();
            }
        }
        KeyCode::Down => {
        ...
        KeyCode::Enter => {
            if let Some(picker) = state.effort_picker.take() {
                let level = picker.selected();
                if level == "off" {
                    state.effort = None;
                } else {
                    state.effort = Some(level.to_string());
                }
                state.chat.add_system(format!("effort: {level}"));
            }
        }
        KeyCode::Esc => {
            state.effort_picker = None;
        }
        _ => {}
    }
    Control::Continue
...
/// 登出选择器按键处理。
async fn handle_logout_picker_key(
    ke:      KeyEvent,
    runtime: &mut BootstrappedRuntime,
    state:   &mut AppState,
) -> Control {
    match ke.code {
        KeyCode::Up => {
            if let Some(picker) = &mut state.logout_picker {
                picker.move_up();
            }
        }
        KeyCode::Down => {
        ...
        KeyCode::Enter => {
            if let Some(picker) = state.logout_picker.take() {
                if let Some(id) = picker.selected_id() {
                    ...
                    state.chat.add_system("no provider selected");
                }
            }
        }
        KeyCode::Esc => {
        ...
        KeyCode::Enter => {
            if let Some(rs) = state.rename_input.take() {
                ...
                if !new_title.is_empty() {
                    ...
                    let _ = ring_core::session::rename_session(id, new_title.clone()).await;
...
fn spawn_fetch_models(
    ...
    tokio::spawn(async move {
        let result = match ring_providers::build_probe_provider(
...
fn spawn_oauth(
    ...
    tokio::spawn(async move {
        let client = ring_providers::provider::build_http_client(
            proxy.as_deref(),
        ...
        let auth_result = if browser {
            ring_providers::providers::openai::oauth::login_browser(&client).await
        } else {
            ring_providers::providers::openai::oauth::login_device(&client).await
        };
        ...
        // 2. 持久化 auth data
        let _ = ring_providers::providers::openai::oauth::save_auth(&auth_data).await;
        ...
        let probe = ring_providers::build_probe_provider(
            proxy.as_deref(),
            &ring_providers::catalog::ProviderKind::OpenAi,
            "openai",
            api_key.clone(),
...
fn build_setup_modal(state: &AppState) -> ProviderSetupModal {
    let catalog = ring_providers::catalog::load(
        Some(&ring_core::session::paths::config_dir()),
...
async fn handle_key(
    ...
    match ke.code {
        ...
        KeyCode::Tab => {
            if !state.suggestions.is_empty() {
                ...
                let next = match runtime.mode {
                    ring_core::ModeName::Ask   => ring_core::ModeName::Edit,
                    ring_core::ModeName::Edit  => ring_core::ModeName::Plan,
                    ring_core::ModeName::Plan  => ring_core::ModeName::Build,
                    ring_core::ModeName::Build => ring_core::ModeName::Agent,
                    ring_core::ModeName::Agent => ring_core::ModeName::Ask,
                };
                runtime.mode = next;
        ...
        // Shift+Tab: 循环 effort 级别（off → minimal → low → medium → high → xhigh → max → off）
        KeyCode::BackTab => {
            const EFFORT_LEVELS: &[&str] = &["off", "minimal", "low", "medium", "high", "xhigh", "max"];
            let current = state.effort.as_deref().unwrap_or("off");
            let idx = EFFORT_LEVELS.iter().position(|&l| l == current).unwrap_or(0);
            let next_idx = (idx + 1) % EFFORT_LEVELS.len();
            let next = EFFORT_LEVELS[next_idx];
            if next == "off" {
                state.effort = None;
            } else {
                state.effort = Some(next.to_string());
            }
            state.chat.add_system(format!("effort: {next}"));
        }
        KeyCode::Esc => {
    ...
    match outcome {
        ...
        CommandOutcome::NewSession => {
            ...
            let messages = {
                ...
                let new_ctx = ring_engine::AgentContext::from_session(
                    &new_session,
                    runtime.model.clone(),
                    Some(runtime.system_prompt.clone()),
                );
        ...
        CommandOutcome::InitAgentsMd => {
            ...
            if agents_path.exists() {
                state.chat.add_system(format!(
                    "AGENTS.md already exists at {}. Ask ring to update it, or delete it first.",
                    agents_path.display()
                ));
                return CmdResult::Handled;
            }
        ...
        CommandOutcome::LoopStart { goal, max_turns } => {
            let ls = ring_core::session::loop_state::LoopState::new(goal.clone(), max_turns);
            state.chat.add_system(format!(
        ...
        CommandOutcome::Reload => {
            ...
            ring_skills::load_skills_from_dir(&mut skills, &global_dir).await;
            ring_skills::load_skills_from_dir(&mut skills, &cwd.join(".agents").join("skills")).await;
            ring_skills::load_skills_from_dir(&mut skills, &cwd.join(".ring").join("skills")).await;
        ...
        CommandOutcome::Handled => {
            ...
            if let Some(rest) = trimmed.strip_prefix("/memory").or_else(|| trimmed.strip_prefix("/mem")) {
                ...
                let entries = if let Some(q) = rest.strip_prefix("search").map(|s| s.trim()).filter(|s| !s.is_empty()) {
                    ring_core::search_memory(q).await
                } else {
                    ring_core::list_memory().await
                };
                if entries.is_empty() {
...
async fn start_turn(
    ...
    let mut executor = ring_engine::agent::orchestrator::build_executor(
...
async fn resume_session_tui(
    ...
    match ring_core::session::load_session(id).await {
        Some(s) => {
            let messages = {
                let mut guard = ctx.lock().await;
                let new_ctx = ring_engine::AgentContext::from_session(
...
async fn delete_session_tui(state: &mut AppState, id: uuid::Uuid) {
    if let Err(e) = ring_core::session::delete_session(id).await {
...
async fn fork_session_tui(
    ...
    match ring_core::session::fork_session(id).await {
        Ok(new_session) => {
...
fn draw<B: ratatui::backend::Backend>(term: &mut Terminal<B>, state: &mut AppState) -> Result<()> {
    term.draw(|frame| {
        ...
        let has_notification = state.status_msg.is_some()
            || token_pct.is_some_and(|p| p >= 70.0);
...
async fn compact_tui(
    ...
    use ring_core::tools::ContentBlock;
    use ring_providers::provider::ChatRequest;
