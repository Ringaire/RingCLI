...
fn welcome_lines<'a>(model: &'a str, mode: &'a str, cwd: &'a str) -> Vec<Line<'a>> {
    ...
    lines.push(Line::from(vec![
        ...
        Span::styled("Welcome to RingCLI", Style::default().fg(UI).add_modifier(Modifier::BOLD)),
    ...
    let tips: &[(&str, &str)] = &[
        ...
        ("Shift+Tab",       "cycle effort: off → minimal → low → medium → high → xhigh → max"),
        ("Ctrl+T",          "toggle extended thinking on/off"),
        ("Ctrl+O",          "toggle reasoning display fold/expand"),
    ...
    let cmds: &[(&str, &str)] = &[
        ...
        ("/model",    "switch model"),
        ("/effort",   "set or cycle reasoning effort"),
