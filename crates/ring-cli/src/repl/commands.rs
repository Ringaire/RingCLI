...
use ring_core::permissions::ModeName;
use ring_core::skills::SkillRegistry;
use ring_core::{delete_memory, list_memory, search_memory};
use uuid::Uuid;
    ...
    match cmd {
        ...
        "effort" => {
            ...
            match level.as_str() {
                ...
                _ => {
                    println!("usage: /effort off|minimal|low|medium|high|xhigh|max");
                    CommandOutcome::Handled
                }
            }
        }
        ...
        "loop" => {
            ...
            let max_cap = ring_core::session::loop_state::MAX_LOOP_TURNS;
            let (goal, max_turns) = {
...
/// 将消息列表渲染为给摘要模型看的文字稿。
pub fn render_compact_transcript(messages: &[ring_core::tools::Message]) -> String {
    use ring_core::tools::MessageRole;
...
fn msg_text(msg: &ring_core::tools::Message) -> String {
...
pub fn split_for_compact(messages: &[ring_core::tools::Message]) -> (Option<String>, &[ring_core::tools::Message]) {
    use ring_core::tools::MessageRole;
    if let Some(first) = messages.first() {
        if first.role == MessageRole::User
            && first.content.iter().any(|b| matches!(b, ring_core::tools::ContentBlock::Text { text } if text.starts_with("Please summarize")))
        {
            if let Some(second) = messages.get(1) {
                let text = msg_text(second);
                if let Some(stripped) = text.strip_prefix("[Conversation Summary]") {
                    let prior = stripped.trim().to_string();
...
/// 若有历史摘要则以 `---` 分隔拼接。
pub fn build_compact_message(prior: Option<&str>, new_summary: &str) -> Vec<ring_core::tools::Message> {
    ...
    vec![
        ring_core::tools::Message::new(MessageRole::User, vec![ContentBlock::Text {
            text: "Please summarize the conversation below into a compact briefing using the requested format.".into()
        }]),
        ring_core::tools::Message::new(MessageRole::Assistant, vec![ContentBlock::Text { text }]),
    ]
}
