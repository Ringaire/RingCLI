        ...
        match cmd_outcome {
            ...
            CommandOutcome::InitAgentsMd => {
                ...
                if agents_path.exists() {
                    println!("[AGENTS.md already exists at {}. Ask ring to update it, or delete it first.]", agents_path.display());
                } else {
            ...
            CommandOutcome::LoopStart { goal, max_turns } => {
                ...
                let mut ls = ring_core::session::loop_state::LoopState::new(goal.clone(), max_turns);
                // 第一轮
                    ...
                    let done = ctx.messages.last()
                        .map(|m| {
                            m.content.iter().any(|b| {
                                if let ring_core::tools::ContentBlock::Text { text } = b {
                                    text.contains(ring_core::LOOP_DONE_MARKER)
                                } else { false }
                            })
                        }).unwrap_or(false);
...
async fn prompt_permission_stdin(
    req:    &ring_engine::PermissionRequest,
    reader: &mut StdinReader,
) -> ring_engine::PermissionDecision {
    use ring_engine::PermissionDecision;
    use tokio::io::AsyncBufReadExt;
    use std::io::Write;
...
fn print_banner(runtime: &BootstrappedRuntime) {
    println!("ring v{} — terminal AI coding assistant", env!("CARGO_PKG_VERSION"));
...
fn build_orchestrator_executor(
    runtime:  &BootstrappedRuntime,
    provider: std::sync::Arc<dyn ring_providers::Provider>,
    perm_tx:  Option<ring_engine::agent::permission::PermissionSender>,
) -> AgentExecutor {
    ring_engine::agent::orchestrator::build_executor(
