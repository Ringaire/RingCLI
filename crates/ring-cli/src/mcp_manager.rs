// McpManager 的 ring-cli 实现：把 MCP server 的工具 + prompts 桥接进 ringcli。
...
//   通过 ring_mcp::import_external_prompts 自动注入。
...
use ring_core::config::McpServerConfig;
use ring_core::runtime::McpManager;
use ring_core::skills::SkillRegistry;
use ring_core::tools::{Tool, ToolContext, ToolRegistry, ToolRegistryExt, ToolResult};

use ring_mcp::{import_external_prompts, McpClient, McpToolBridge, SseTransport, StdioTransport, Transport};
