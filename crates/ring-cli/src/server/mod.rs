//! HTTP server 模式：`ring --serve 127.0.0.1:8765`
//!
//! REST API：
//! - `GET  /health` — 健康检查
...
pub async fn serve(addr: &str) -> anyhow::Result<()> {
    ...
    let _bind_addr = if addr.contains(':') {
        addr.to_string()
...
struct ServerState {
    provider:      Option<Arc<dyn ring_providers::provider::Provider>>,
    tools:         Arc<dyn ring_core::tools::ToolRegistry>,
    permissions:   Arc<tokio::sync::Mutex<ring_core::permissions::DefaultPermissionEngine>>,
    bus:           ring_core::events::EventBus,
    catalog:       Vec<ring_core::agent::ModelCatalogEntry>,
    model:         String,
    system_prompt: String,
    cwd:           std::path::PathBuf,
    session:       ring_core::session::Session,
}
...
async fn chat(
    ...
    ctx.add_message(ring_core::tools::Message::user_text(&req.prompt));
    ...
    let executor = ring_engine::agent::orchestrator::build_executor(
        ring_engine::agent::orchestrator::ExecutorParams {
            ...
            session_id: state.session.meta.id,
            cwd: state.cwd.clone(),
            ...
            let messages: Vec<serde_json::Value> = session
                ...
                .map(|m| {
                    let role = match m.role {
                        ring_core::tools::MessageRole::User => "user",
                        ring_core::tools::MessageRole::Assistant => "assistant",
                        ring_core::tools::MessageRole::ToolResult => "tool",
                    };
                    let text: String = m
                        .content
                        .iter()
