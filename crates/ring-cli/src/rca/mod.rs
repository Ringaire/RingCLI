            ...
            msg = async {
                ...
                match msg {
                    ...
                    Some(Err(e)) => {
                        ...
                        if let Some((_, _session_id)) = ws.take() {
...
> {
    ...
    let register = Envelope {
        ...
        payload: serde_json::to_value(Register {
            ...
            labels: Some(vec!["ringcli".to_string()]),
            auth_token,
        })
        .unwrap_or_else(|e| {
...
pub async fn connect_and_run(
    ...
    // 注册
    let client_id = format!("ring-{}", &Uuid::new_v4().to_string()[..8]);
    ...
    let (hb_tx, mut hb_rx) = mpsc::unbounded_channel::<Message>();
    {
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                if hb_tx.send(Message::Text(text)).is_err() {
                    break;
                }
            }
        });
...
async fn execute_task(
    ...
    ctx.add_message(ring_core::tools::Message::user_text(&task.message.text));
