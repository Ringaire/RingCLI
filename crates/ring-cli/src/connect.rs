...
pub async fn switch_model(runtime: &mut BootstrappedRuntime, model_ref: &str) -> SwitchResult {
    let (prov_hint, model_name) = ring_providers::split_model_ref(model_ref);
    match prov_hint {
...
pub async fn quick_connect(
    ...
    let Some(entry) = ring_providers::catalog::get(&catalog, provider).cloned() else {
...
/// `/logout [provider]`：清除全局配置中存储的 api_key。
///
/// - 只清除文件里持久化的 key，不碰环境变量。
/// - 如果登出的是当前活跃 provider，断开连接（runtime.provider = None）。
/// - `provider_id` 为 None 时返回已存储凭据的 provider 列表供 picker 选择。
///
/// 返回 `(provider_id, model)` 表示清除成功；返回 None 表示该 provider 无存储凭据。
pub async fn logout_provider(
    ...
    let _guard = config_write_lock().lock().await;
    let mut cfg = load_user_config().await;
    let providers = match cfg.providers.as_mut() {
        Some(p) => p,
        None => return Err(format!("no provider '{provider_id}' in config")),
    };
    let entry = match providers.get_mut(provider_id) {
        Some(e) => e,
        None    => return Err(format!("no provider '{provider_id}' in config")),
    };
    ...
    let boot = ring_providers::build_registry(&resolved);
    let registry = Arc::new(boot.registry);
    let new_provider = registry.get(provider_id);

    runtime.provider_registry = registry;
    runtime.config = resolved;

    if was_active {
        // 该 provider key 被清了，registry 中已无可用实例 → 断开
        runtime.provider = new_provider;
        if runtime.provider.is_none() {
            runtime.model = String::new();
        }
        runtime.rebuild_context().await;
    }
...
/// 列出全局配置中存储了 api_key 的 provider（供 `/logout` picker 使用）。
pub fn list_stored_credentials() -> Vec<(String, String)> {
    // 同步读取（picker 构造时无法 await），用 tokio::runtime::Handle 或 spawn_blocking 均可。
    // 但 picker 在 TUI 线程中构造，最简方案：用 std::fs。
    let path = ring_core::session::paths::config_path();
