...
pub fn parse_jsonc<T: for<'de> Deserialize<'de>>(src: &str) -> Result<T, serde_json::Error> {
...
fn utf8_char_len(byte: u8) -> usize {
    if byte < 0x80 { 1 }
    else if byte < 0xE0 { 2 }  // 0xC0-0xDF: 2字节字符
    else if byte < 0xF0 { 3 }  // 0xE0-0xEF: 3字节字符
    else { 4 }                 // 0xF0+: 4字节字符
    // 注：0x80-0xBF 是续接字节，不应出现在起始位置，这里容错返回1
}
...
pub async fn load_config(cwd: Option<&std::path::Path>) -> ResolvedConfig {
    ...
    // 2b. 项目级 cwd/tool.json（新统一格式）
    if let Some(dir) = cwd {
        let tool_path = dir.join("tool.json");
        if let Ok(raw) = tokio::fs::read_to_string(&tool_path).await {
            let mcp = parse_mcp_json(&raw);
            if !mcp.is_empty() {
                let target = merged.mcp_servers.get_or_insert_with(HashMap::new);
                for (k, v) in mcp {
    ...
    // 2c. 项目级 cwd/mcp_server.json（旧格式兼容）
    if let Some(dir) = cwd {
        let mcp_path = dir.join("mcp_server.json");
        if let Ok(raw) = tokio::fs::read_to_string(&mcp_path).await {
            let mcp = parse_mcp_json(&raw);
            if !mcp.is_empty() {
                let target = merged.mcp_servers.get_or_insert_with(HashMap::new);
                for (k, v) in mcp {
    ...
    if let Some(dir) = cwd {
        ...
        for fname in ["tool.json", "mcp_server.json", ".mcp.json"] {
            let path = ring_dir.join(fname);
            if let Ok(raw) = tokio::fs::read_to_string(&path).await {
                let mcp = parse_mcp_json(&raw);
                if !mcp.is_empty() {
                    let target = merged.mcp_servers.get_or_insert_with(HashMap::new);
    ...
    // 2e. 全局 ~/.ring/config/ 下 tool.json / mcp_server.json
    {
        for fname in ["tool.json", "mcp_server.json", ".mcp.json"] {
            let path = paths::config_dir().join(fname);
            if let Ok(raw) = tokio::fs::read_to_string(&path).await {
                let mcp = parse_mcp_json(&raw);
                if !mcp.is_empty() {
                    let target = merged.mcp_servers.get_or_insert_with(HashMap::new);
