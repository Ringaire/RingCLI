...
//! - 类型安全的工具查找键
//!
//! # 与 ring-tools::BuiltinTool 的关系
//!
//! 由于依赖方向 `ring-tools → ring-core`，`ring-core` 无法持有具体工具类型。
//! 因此：
//! - **本 crate（ring-core）**：定义 `BuiltinToolKind` 类型标识
//! - **ring-tools crate**：定义 `BuiltinTool` 枚举，持有具体工具实例（unit struct），
//!   通过 `BuiltinTool::from_name(kind.name())` 与本枚举建立映射
//!
//! 实际的工具分发枚举在 `ring_tools::builtin::BuiltinTool`。
...
/// 1. 在此枚举添加变体
/// 2. 在 `name()` 添加映射
/// 3. 在 `ring_tools::BuiltinTool` 添加对应变体
...
impl BuiltinToolKind {
    ...
    /// 占位：未来用于工具元数据查询。
    ///
    /// 当前返回空 `Value`，具体 schema 由 `ring_tools::BuiltinTool::input_schema` 提供。
    #[deprecated(note = "use ring_tools::BuiltinTool::input_schema via registry lookup")]
