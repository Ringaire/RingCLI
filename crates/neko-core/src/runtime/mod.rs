...
// 配置热重载。
//
// 注：core 不能依赖 neko-mcp（否则 mcp→core→mcp 循环）。因此 MCP 的具体加载
// 通过 `McpManager` trait 做依赖反转 —— 由上层（neko-cli）注入实现。这是 Rust
// 中对 bun 版「动态 import('@nekocode/mcp')」那一 hack 的等价、且类型安全的写法。
