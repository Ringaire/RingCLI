...
[功能特性](#功能特性) • [快速开始](#快速开始) • [架构设计](#架构设计) • [支持的提供商](#支持的-llm-提供商) • [路线图](#路线图) • [English](README_EN.md)
...
- 🖥️ **TUI 终端界面** — 基于 React/ink，支持 Markdown 渲染、流式输出、推理展示
...
│   ├── src/tui/      ── React/ink 组件（App, MessageList, PromptInput）
│   ├── src/repl/     ── REPL 命令处理、模式切换
│   └── src/input/    ── 输入解析、命令补全、@提及
│
├── core/           ── 核心引擎
│   ├── agent/        ── 模型选择器、角色分类
│   ├── config/       ── 配置加载、路径管理
...
│   └── openai-compatible/ ── DeepSeek, Groq, SiliconFlow 等
│
├── tools/          ── 15 个内置工具实现
│   ├── bash/         ── Shell 命令执行
│   ├── file/         ── 读/写/编辑/树
...
- 参考项目：
    ...
    - [NekoBot](https://github.com/Carillen/NekoBot) — 框架设计、插件系统
- 所有贡献者
